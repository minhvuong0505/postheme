...
function get_item_data_row($item)
{
	...
	if($item->pic_filename != '')
	{
		...
		if(sizeof($images) > 0)
		{
			$image .= '<a class="rollover" href="'. base_url($images[0]) .'" target="_blank"><img src="'.site_url('items/pic_thumb/' . pathinfo($images[0], PATHINFO_BASENAME)) . '"></a>';
		}
	}
	...
	return array (
		...
		'name' => $item->name,
		'promotion_day_start' => $item->promotion_day_start == NULL ? NULL : date('m/d/y',$item->promotion_day_start),
		'promotion_day_end' =>  $item->promotion_day_end == NULL ? NULL : date('m/d/Y',$item->promotion_day_end),
		'promotion_price' =>  $item->promotion_price == NULL ? NULL : $item->promotion_price,
